...
namespace osj
{
    Mesh generate_inrun_mesh(const Hill &hill, const Transform &t)
    {
        ...
        {
            mesh.vertices.push_back(t.transform_point({inrun_points[i], -b1 / 2}));
            mesh.vertices.push_back(t.transform_point({inrun_points[i], b1 / 2}));
        ...
        {
            mesh.triangles.push_back(2 * i + 0);
            mesh.triangles.push_back(2 * i + 3);
            mesh.triangles.push_back(2 * i + 1);
            mesh.triangles.push_back(2 * i + 0);
            mesh.triangles.push_back(2 * i + 2);
            mesh.triangles.push_back(2 * i + 3);
        }
        return mesh;
    }

    Mesh generate_lhill_mesh(const Hill &hill, const Transform &t)
    {
        ...
        for (size_t i = 0; i < lhill_points.size(); i++)
        {
            ...
            mesh.vertices.push_back(t.transform_point({lhill_points[i].x, lhill_points[i].y, -half_w}));
            mesh.vertices.push_back(t.transform_point({lhill_points[i].x, lhill_points[i].y, half_w}));
